...
const TABS = ['FEES', 'Kushki', 'Banregio', 'Conciliaciones']
const EXPORT_CONFIG = [
  { label: 'Exportar FEES', fn: resultsApi.exportFees },
...
function FeesTab({ processId }) {
  ...
  if (isLoading) return <Loader2 size={20} className="animate-spin text-slate-500 mx-auto mt-8" />
  if (!data) return <p className="text-slate-500 text-sm text-center mt-8">Sin resultados FEES aún</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total FEES', value: data.total_fees },
          { label: 'Merchants', value: data.merchant_summary?.length || 0, isMoney: false },
      ...
      {chartData.length > 0 && (
        <div className="card">
          <h3 className="text-sm font-semibold text-white mb-4">Top 10 merchants por fee</h3>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-800">
          <h3 className="text-sm font-semibold text-white">Resumen por merchant</h3>
        </div>
        <table className="w-full text-sm">
          <thead className="border-b border-slate-800 bg-slate-900/50">
            <tr>
              {['Merchant', 'Transacciones', 'Monto bruto', 'Total fee'].map(h => (
          ...
          <tbody>
            {(data.merchant_summary || []).map((m, i) => (
              <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                <td className="px-5 py-2.5 text-slate-200">{m.merchant_name || m.merchant_id}</td>
                <td className="px-5 py-2.5 text-slate-400">{m.tx_count?.toLocaleString()}</td>
                <td className="px-5 py-2.5 text-slate-300"><FmtMoney value={m.gross_amount} /></td>
                <td className="px-5 py-2.5 text-brand-400 font-medium"><FmtMoney value={m.total_fee} /></td>
              </tr>
            ))}
          </tbody>
        </table>
...
function KushkiTab({ processId }) {
  ...
  if (isLoading) return <Loader2 size={20} className="animate-spin text-slate-500 mx-auto mt-8" />
  if (!data) return <p className="text-slate-500 text-sm text-center mt-8">Sin resultados Kushki</p>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div className="card text-center">
          <p className="text-2xl font-bold text-white"><FmtMoney value={data.total_net_deposit} /></p>
          <p className="text-sm text-slate-400 mt-1">Net Deposit Total</p>
        </div>
        <div className="card text-center">
          <p className="text-2xl font-bold text-white">{data.daily_summary?.length || 0}</p>
          <p className="text-sm text-slate-400 mt-1">Días procesados</p>
        </div>
      </div>

      {chartData.length > 0 && (
        <div className="card">
          <h3 className="text-sm font-semibold text-white mb-4">Depósito neto diario</h3>
          <ResponsiveContainer width="100%" height={220}>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-800">
          <h3 className="text-sm font-semibold text-white">Resumen diario</h3>
        </div>
        <table className="w-full text-sm">
          <thead className="border-b border-slate-800 bg-slate-900/50">
            <tr>
          ...
          <tbody>
            {(data.daily_summary || []).map((d, i) => (
              <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                ...
                <td className="px-4 py-2.5 text-slate-400">{Number(d.tx_count || 0).toLocaleString()}</td>
                <td className="px-4 py-2.5 text-slate-300"><FmtMoney value={d.gross_amount} /></td>
                <td className="px-4 py-2.5 text-amber-400"><FmtMoney value={d.commission} /></td>
                <td className="px-4 py-2.5 text-slate-400"><FmtMoney value={d.rolling_reserve} /></td>
                <td className="px-4 py-2.5 text-emerald-400 font-medium"><FmtMoney value={d.net_deposit} /></td>
              </tr>
            ))}
          </tbody>
        </table>
...
function BanregioTab({ processId }) {
  ...
  if (isLoading) return <Loader2 size={20} className="animate-spin text-slate-500 mx-auto mt-8" />
  if (!data) return <p className="text-slate-500 text-sm text-center mt-8">Sin resultados Banregio</p>
    ...
    <div className="space-y-6">
      ...
      <div className="card p-0 overflow-hidden">
        ...
        <div className="max-h-96 overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-slate-800 bg-slate-900/50 sticky top-0">
              <tr>
            ...
            <tbody>
              {(data.movements || []).map((m, i) => (
                <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                  <td className="px-4 py-2.5 text-slate-400 text-xs">{m.date}</td>
                  <td className="px-4 py-2.5 text-slate-300 max-w-xs truncate">{m.description}</td>
                  <td className="px-4 py-2.5 text-red-400"><FmtMoney value={m.debit} /></td>
                  <td className="px-4 py-2.5 text-emerald-400"><FmtMoney value={m.credit} /></td>
                  <td className="px-4 py-2.5 text-slate-400"><FmtMoney value={m.deposit_ref} /></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function ConciliationTab({ processId }) {
  ...
  if (isLoading) return <Loader2 size={20} className="animate-spin text-slate-500 mx-auto mt-8" />
  if (!data.length) return <p className="text-slate-500 text-sm text-center mt-8">Sin conciliaciones aún</p>
      ...
      {data.map(c => (
        <div key={c.id} className="card">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-white">{TYPE_LABEL[c.conciliation_type] || c.conciliation_type}</h3>
          ...
          <div className="grid grid-cols-4 gap-3 mb-4">
            {[
              { label: 'Conciliados', value: c.matched?.length || 0, color: 'text-emerald-400' },
              { label: 'Diferencias', value: c.differences?.length || 0, color: 'text-amber-400' },
              { label: 'Sin match Kushki', value: c.unmatched_kushki?.length || 0, color: 'text-red-400' },
              { label: 'Sin match Banregio', value: c.unmatched_banregio?.length || 0, color: 'text-red-400' },
            ].map(s => (
                ...
                <thead className="border-b border-slate-800">
                  <tr>
                    {['Fecha', 'Kushki (Col I)', 'Banregio (Col H)', 'Diferencia'].map(h => (
                      <th key={h} className="text-left px-3 py-2 text-slate-400">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {c.matched.map((m, i) => (
                    <tr key={i} className="border-b border-slate-800/50">
                      ...
                      <td className="px-3 py-2 text-slate-300"><FmtMoney value={m.kushki_amount} /></td>
                      <td className="px-3 py-2 text-slate-300"><FmtMoney value={m.banregio_amount} /></td>
                      <td className={`px-3 py-2 ${m.difference > 0 ? 'text-amber-400' : 'text-emerald-400'}`}>
    ...
    <div className="p-8 space-y-6">
      <div>
        <div className="flex items-center gap-2 text-slate-500 text-sm mb-1">
          ...
          <Link to={`/processes/${id}`} className="hover:text-slate-300">{proc?.name}</Link>
          <ChevronRight size={12} />
          <span className="text-slate-300">Resultados</span>
        </div>
        <h1 className="text-2xl font-bold text-white">Resultados del cierre</h1>
        {proc && (
